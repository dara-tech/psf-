                    ...
                    @if (count($users) > 0)
                        @foreach ($users as $user)
                            <tr data-entry-id="{{ $user->id }}">
                                ...
                                <td>
                                    @foreach ($user->sites()->pluck('sitename') as $role)
                                        <span class="label label-info label-many">{{ $role }}</span>
                                    @endforeach
                                </td>
                                <td>
                                    ...
                                    {!! Form::open(array(
                                        ...
                                        'method' => 'POST',
                                        'onsubmit' => "return confirm('".trans("global.app_are_you_sure")."');",
                                        'route' => ['admin.users.resetpwd', $user->id])) !!}
                                    {!! Form::submit(trans('Reset Password'), array('class' => 'btn btn-xs btn-danger')) !!}
                                    {!! Form::close() !!}
